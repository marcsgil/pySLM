...
class BeamSection:
    def __init__(self, grid: Grid, propagation_axis: int = -3,
        ...
        Note that the polarizations should be specified in the same order as the zyx-grid axes, i.e. Ez, Ey, Ex. If you
        prefer to use the order x-y-z for both and propagate in the right-most dimension, set the propagation_axis
        argument to -1. The default, -3, is chosen for efficiency.
    ...
    def __iter__(self, distance: float = None, nb_steps: int = None) -> Generator[BeamSection]:
        if nb_steps is None:
            nb_steps = np.inf
...
class Beam:
    ...
    def __iter__(self, permittivity: Optional[array_type] = None, refractive_index: Optional[array_type] = None,
                 infinite: bool = False) -> Generator[BeamSection, None, None]:
        """
        ...
        while infinite:  # Continue generating beam sections when instructed to do so.
            beam_section.propagate(distance=distance)
