...
if __name__ == '__main__':
    ...
    try:
        ...
        if num_boards_found.value == 1:
            ...
            phases_on_meadowlark = np.zeros(slm_shape)
            ...
            phases_on_meadowlark_gl = np.empty([slm_height, slm_width], np.uint8, 'C')
            ...
            phase = 0
            center_adjustment = 20
            while phase is not None:
                phases_on_meadowlark[:, (phases_on_meadowlark.shape[1] // 2) + center_adjustment:] = phase
                phases_on_meadowlark_gl[:] = (128 + phases_on_meadowlark * (128 / np.pi)).astype(np.uint8)
                slm_lib.Write_image(board_number, phases_on_meadowlark_gl.ctypes.data_as(POINTER(c_ubyte)), int(np.prod(slm_shape)),
                ...
                #center_adjustment = input('Center adjustment:') #todo remove after the alligment
                input_phase_str = input('Display a phase in degree:')
                try:
                    phase = np.deg2rad(float(input_phase_str))
                except ValueError:
                    phase = None
