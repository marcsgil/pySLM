...
model = model.cpu()
#
rng = np.random.Generator(np.random.PCG64())  # For the noise generation
if model_idx < 3:
    nb_modes = 18
    ...
    with IDSCam(index=1, normalize=True, exposure_time=10e-3, gain=1, black_level=110) as cam:
        with AlpaoDM(serial_name='BAX240') as dm:
            ...
            def modulate_and_capture(coefficients) -> float:
                ...
                abersize = coefficients.size
                print(abersize)
                aberration[0, 0:abersize] = coefficients
                mirror_command = np.dot(aberration, Z2CMatrix)
                dm.modulate(mirror_command * 20)
                # actuator_positions = dm.actuator_position
                log.info(f'Displaying {aberration}...')
                img = cam.acquire()
                img_v = np.fliplr(img)
                img_hv = np.flipud(img)
                time.sleep(0.1)
                return img_hv  # pick the maximum (might be noisy)
