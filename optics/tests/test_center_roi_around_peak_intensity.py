...
class TestCenterRoiAroundPeakIntensity(unittest.TestCase):
    def setUp(self):
        noise_level = 0.50
        ...
        def simulate_image(c: Cam, pos=None, brightness=1.0):
            ...
            sim_img[200, 300] = 0.9  # add second intense pixel, 90% of the first's intensity
    ...
    def test_second_peak(self):
        self.cam_5_5.roi = Roi(190, 290, 32, 32)  # so that the second peak, at (200, 300), is the only one
        self.cam_5_5.center_roi_around_peak_intensity()
        npt.assert_array_equal(self.cam_5_5.roi.center, np.array((200, 300)),
                               "Not centered around peak intensity.")
