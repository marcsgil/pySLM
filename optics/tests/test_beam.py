...
class TestBeamSection(unittest.TestCase):
    def setUp(self):
        ...
        self.n0 = 1.2
        self.epsilon0n2 = self.n0 ** 2
        self.k0n = self.n0 * 2 * np.pi / self.vacuum_wavelength  # NOT in vacuum
        ...
        self.polarizations = [1, np.array([0, 1, 0]), np.array([0, 0, 1]), np.array([0, 1, 1j]) / np.sqrt(2)]
        # todo: test suppression of polarization in the propagation direction
    ...
    def test_constructor_field_ft(self):
        for polarization in self.polarizations:
            for grid in self.section_grids:
                ...
                beam_section = BeamSection(grid, vacuum_wavelength=self.vacuum_wavelength, background_permittivity=self.epsilon0n2, field_ft=field_ft)
                ...
                npt.assert_almost_equal(beam_section.wavenumber, self.k0n, err_msg=f'wavenumber incorrectly set or reported for {desc}.')
                npt.assert_almost_equal(beam_section.vacuum_wavenumber, self.k0n / self.n0, err_msg=f'vacuum wavenumber incorrectly set or reported for {desc}.')
                ...
                npt.assert_equal(beam_section.background_permittivity, self.epsilon0n2, err_msg=f'background permittivity incorrectly set or reported for {desc}.')
        ...
        for polarization in self.polarizations:
            for grid in self.section_grids:
                ...
                beam_section = BeamSection(grid, vacuum_wavelength=self.vacuum_wavelength,
                                           background_permittivity=self.epsilon0n2, field=field)
                while field.ndim < 3 + 1:  # Check if singleton dimension added automatically
                    field = field[np.newaxis, ...]
                    field_ft = field_ft[np.newaxis, ...]
                npt.assert_array_equal(beam_section.vectorial, vectorial, err_msg=f'BeamSection.field_ft not set correctly for {desc}.')
            ...
            for grid in self.section_grids:
                ...
                for field in fields:
                    ...
                    if not vectorial or np.asarray(field).ndim == grid.ndim:  # If vectorial, only consider correctly-dimensioned inputs
                        if vectorial:
                            field = add_dims_on_right(polarization, 3) * field
                        beam_section = BeamSection(grid, vacuum_wavelength=self.vacuum_wavelength, background_permittivity=self.epsilon0n2, field=field)
    ...
    def test_plane_wave_normal_incidence(self):
        for polarization in self.polarizations:
            for grid in self.section_grids:
                ...
                npt.assert_array_almost_equal(beam_section.field, field * np.exp(1j * self.k0n * distance), err_msg=f'BeamSection.field not propagated correctly for {desc}.')
                npt.assert_array_almost_equal(beam_section.field_ft, field_ft * np.exp(1j * self.k0n * distance), err_msg=f'BeamSection.field_ft not propagated correctly for {desc}.')
    ...
    def test_refractive_index_normal_incidence(self):
        for polarization in self.polarizations:
            for grid in self.section_grids:
                ...
                phasor = np.exp(1j * self.k0n * (1 + (refractive_index / self.n0 - 1)) * distance)  # first refract, then propagate
        ...
        for polarization in self.polarizations:
            for section_grid in self.section_grids:
                if section_grid.ndim > 0:  # Skip 1-dimensional calculations
                    ...
                    for kt in kts:
                        if kt < self.k0n:  # Do not check evanescent components
                            kz = np.sqrt(1.0 - (kt / self.k0n) ** 2) * self.k0n
                            desc = f'calculation on {section_grid.shape}-grid with polarization {polarization} at {np.arcsin(kt / self.k0n) * 180 / np.pi:0.1f} degrees.'
                            ...
                            beam_section = BeamSection(section_grid, vacuum_wavelength=self.vacuum_wavelength,
                                                       background_permittivity=self.epsilon0n2, field_ft=field_ft,
                            ...
                            beam_section.propagate(distance, refractive_index=refractive_index)
                            phasor = np.exp(1j * (kz + self.k0n * (refractive_index / self.n0 - 1)) * distance)  # Refraction is applied instantaneously (angle independent), propagation is angle dependent
                ...
                if section_grid.ndim > 0:  # Skip 1-dimensional calculations
                    ...
                    for kt in kts:
                        if kt < self.k0n:  # Do not check evanescent components
                            ...
                            field = np.exp(1j * kt * section_grid[0])
                            field = np.broadcast_to(field, (*field.shape[:-section_grid.ndim], *section_grid.shape))
                            ref_field = np.full(field.shape, 1.23j)
                            vectorial = np.asarray(polarization).size > 1
                            if vectorial:  # Rotate polarization vector and change scalar field to vectorial
                                c, s = kz / self.k0n, kt / self.k0n
                                rot = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]])
                                ...
                                rotated_polarization = (rot @ polarization[:, np.newaxis])[:, 0]
                                field = add_dims_on_right(rotated_polarization, 3 + 1) * field
                                ref_field = add_dims_on_right(polarization, 3 + 1) * ref_field
                            shifted_field = np.array(field)
                            for f, ax in zip(section_grid.first, transverse_ft_axes):
                                if f != 0:
                                    shifted_field = ft.ifftshift(shifted_field, axes=ax)
                            field_ft = ft.fftn(shifted_field, axes=transverse_ft_axes % field.ndim) if section_grid.ndim > 0 else field
                            ref_field_ft = ft.fftn(ref_field, axes=transverse_ft_axes % field.ndim) if section_grid.ndim > 0 else ref_field
                            beam_section = BeamSection(section_grid, vacuum_wavelength=self.vacuum_wavelength,
                                                       background_permittivity=self.epsilon0n2, field_ft=field_ft + ref_field_ft,
                                                       propagation_axis=propagation_axis)
                            while field.ndim < 3 + 1:  # When scalar, singleton dimensions should have been added
                                field = field[np.newaxis]
                                field_ft = field_ft[np.newaxis]
                            npt.assert_array_almost_equal(beam_section.field_ft, field_ft + ref_field_ft, err_msg=f'BeamSection.field_ft not set correctly for {desc}.')
                            npt.assert_array_almost_equal(beam_section.field, field + ref_field, err_msg=f'BeamSection.field not set correctly for {desc}.')
                            # Propagate forward and check
                            distance = self.vacuum_wavelength
                            beam_section.propagate(distance)
                            phasor = np.exp(1j * kz * distance)  # propagate only
                            ref_phasor = np.exp(1j * self.k0n * distance)
                            npt.assert_array_almost_equal(beam_section.field_ft, field_ft * phasor + ref_field_ft * ref_phasor, err_msg=f'BeamSection.field_ft not propagated correctly for {desc}.')
                            npt.assert_array_almost_equal(beam_section.field, field * phasor + ref_field * ref_phasor, err_msg=f'BeamSection.field not propagated correctly for {desc}.')
                            beam_section.propagate(-distance) # Reset and try with a fractional distance and refractive index
                            distance = self.vacuum_wavelength * np.pi
                            refractive_index = 1.5 + 0.1j
                            beam_section.propagate(distance, refractive_index=refractive_index)
                            phasor = np.exp(1j * (kz + self.k0n * (refractive_index / self.n0 - 1)) * distance)  # Refraction is applied instantaneously (angle independent), propagation is angle dependent
                            ref_phasor = np.exp(1j * self.k0n * refractive_index / self.n0 * distance)
                            npt.assert_array_almost_equal(beam_section.field_ft, field_ft * phasor + ref_field_ft * ref_phasor, err_msg=f'BeamSection.field_ft not propagated correctly at angle through glass for {desc}.')
                            npt.assert_array_almost_equal(beam_section.field, field * phasor + ref_field * ref_phasor, err_msg=f'BeamSection.field not propagated correctly at angle through glass for {desc}.')
                            beam_section.propagate(-distance, refractive_index=refractive_index)
                            npt.assert_array_almost_equal(beam_section.field_ft, field_ft + ref_field_ft, err_msg=f'BeamSection.field_ft not propagated back correctly at angle through glass for {desc}.')
                            npt.assert_array_almost_equal(beam_section.field, field + ref_field, err_msg=f'BeamSection.field not propagated back correctly at angle through glass for {desc}.')
...
class TestBeam(unittest.TestCase):
    ...
    def test_propagation_along_different_axis(self):
        for grid in self.grids:
            if grid.ndim > 1:
                for prop_axis in range(-grid.ndim, 0):
                    for polarization in self.polarizations:
